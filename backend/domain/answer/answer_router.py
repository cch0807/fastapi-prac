...
from backend.domain.answer import answer_schema, answer_crud
...
def answer_delete(
    ...
    if not db_answer:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="답변을 찾을 수 없습니다."
        )

    if current_user.id != db_answer.user.id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="답변 삭제 권한이 없습니다."
        )
